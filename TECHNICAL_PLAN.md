...
| D4 | Packages/bundles ("Puppy first-year: 4 visits + 3 vaccines") | P1 | 🔨 | `Package`/`PackageRepository` + `PackagesView` + `0015_packages.sql` ship; buying one (`BuyPackageUseCase`) still just expands into N `CartItem` lines booked/scheduled together at purchase time. True "3 of 4 visits used" redemption tracking is blocked on a real structural gap, not left undone by omission: `Visit` carries no `serviceId`/package reference at all (only `petId`/`vetId`/`circuitId`), so there is nothing today to tie a completed visit back to "which package entitlement did this consume" without adding that linkage across the booking/checkout pipeline — out of scope for this pass to bolt on safely |
