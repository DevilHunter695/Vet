...
| E6 | **Server-authoritative quote**: `POST /quotes` returns a signed, TTL'd quote; order must reference a valid quote | **P0** | 🔨 | The signed/TTL'd quote half was already done (`create-quote` Edge Function). This pass closed the enforcement half at the type level: `StartCheckoutUseCase.execute` now takes a `Quote` (there is no raw-amount overload), rejects an expired one, and derives the charged amount from `quote.breakdown.totalMinorUnits`; `Payment.quoteId` (0051_payments_quote_id.sql) records which quote authorized it — `payments` is this app's order table, there is no separate `orders` entity. Stays 🔨 because it isn't reachable end-to-end yet: `BookingView` (the main "tap a circuit → book" flow) creates a `Visit` with no quote or payment step at all, and `CartView`'s quote flow never creates a visit or calls checkout — the two booking pipelines are disconnected from each other and neither drives a real payment today. Wiring either into `StartCheckoutUseCase` is a materially larger redesign (Visit also has no `serviceId`/cart linkage at all, the same structural gap noted on D4) than is safe to bolt on in this pass |
...
| E8 | Payment method choice: UPI intent, cards, netbanking, wallets, **pay-after-visit (cash/UPI to vet)** | P0 | 🔨 | UPI/cards/netbanking/wallets are, by design, Razorpay hosted-checkout's own page (G1: "never raw card data" means the app never builds that picker itself) — that part is covered by construction once checkout actually runs. "Pay-after-visit" genuinely isn't modeled anywhere (no field, no UI, no `Payment` case) — and there's nowhere real to wire it into: per E6's note, no booking flow in this app calls checkout at all today, so a payment-method choice has no checkout screen to be a choice *on*. Building the toggle without the checkout step behind it would be a decoration, not a feature |
| E9 | Saved payment methods (gateway-tokenized, never stored by you) | P1 | ✅ | |
| E10 | Order confirmation screen + receipt email/SMS | P0 | 🔨 | The confirmation screen exists (`BookingConfirmedView`); this pass added the receipt half that was pure dead code before it — `SendTransactionalNotificationUseCase` (J8) existed but was never called from anywhere in the app. `BookingViewModel.confirmBooking` now calls it on success (category `.visitConfirmed`), so a real push-or-SMS-fallback decision actually fires and is logged, and the confirmation screen says so. Stays 🔨: "SMS" is real (fallback decision + `sms_fallback_log`, per J8's own scope — no live gateway sends an actual text, same documented limit as everywhere else this mechanism is used); "email" has no email-sending path anywhere in this codebase at all (no SMTP/provider integration exists to build on) |
