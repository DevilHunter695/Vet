...
| O4 | Accessibility: Dynamic Type to AX5, VoiceOver, Reduce Motion | P0 | 🔨 | Reduce Motion is genuinely wired into the shared animation primitives every screen builds on (`Theme.appearAnimation`, `Mascot`, `Confetti` all read `\.accessibilityReduceMotion` and skip/short-circuit their animation); VoiceOver labels exist on icon-only controls in several views (chat attach/send, share buttons, etc). Text sizing relies on SwiftUI's default Dynamic Type scaling rather than fixed point sizes, so it isn't broken, but there's no deliberate AX1-AX5 layout audit (no screen has been checked for clipping/truncation/overlap at the largest accessibility sizes) and no VoiceOver rotor/focus-order pass across the ~50 screens in this app — that audit is real, non-trivial work this pass didn't have the scope to do screen-by-screen and is the honest remaining gap |
