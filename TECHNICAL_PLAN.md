...
| A7 | Export my data (JSON + PDF of records) | P1 | ✅ | DPDP data-principal right — JSON export plus a PDF summary (`DataExport.summaryPDF()`, `PrivacyConsentView`), reusing the `PDFDocumentBuilder` pattern from K2/K4 |
...
| A9 | Household: invite spouse/family to same pets & bookings | P1 | ✅ | Roles: owner/member. `Household`/`HouseholdMember` + `HouseholdRepository`, `HouseholdView` linked from Profile. Pets keep `owner_id`; visibility only, via RLS (0020_households.sql). Bookings sharing (the gap that kept this 🔨) closed by 0060_household_visit_sharing.sql (household-scoped `visits` select/cancel policies) + `ManageHouseholdUseCase.sharedPets`/`sharedVisits`, both composing the existing `PetRepository`/`VisitRepository` and shown in `HouseholdView`. Remaining honest limit, shared with `Referral`/`CorporateSeatAssignment`: invite is by phone number only, with no real SMS delivery and no server-side trigger yet to match that phone to a real `user_id` on signup — the invited row sits pending until that matching lands, same as those two features |
...
| # | Capability | Pri | Status | Notes |
|---|---|---|---|---|
| B1 | Multiple pets per household, add/**edit**/**delete**/archive | P0 | ✅ | Add/edit (`ManagePetsUseCase.add`/`.update`) and archive (B8) were already done; the remaining gap was the pet list's swipe-to-delete calling a hard `deletePet` that would orphan visit/vaccination history — `ProfileView`'s swipe action now archives (`.other` reason) instead of hard-deleting, matching B8's soft-delete discipline |
| B2 | Pet detail screen: photo, species, breed, DOB, sex, neutered, weight, microchip, allergies, chronic conditions | P0 | ✅ | `PetDetailView` covers every field; photo upload closed this pass — `Pet.photoURL` (0046_pet_photo_url.sql) + `PetRepository.updatePhoto` (Mock + Supabase) + a `PhotosPicker` in the pet-info card. Supabase side writes the `photo_url` reference column for real; pushing the actual JPEG bytes to Storage is a documented TODO, same discipline already accepted for B6's document vault |
| B3 | Weight & vitals history (chart) | P1 | ✅ | `pet_weights` table + `PetWeightRepository` + Swift Charts line chart in `PetDetailView`; vitals beyond weight closed this pass — optional `temperatureCelsius`/`heartRateBpm` on `PetWeightEntry` (0047_pet_vitals.sql), plausibility-validated in `ManagePetWeightsUseCase.addEntry` (unit-tested), entered in the same "Add weight" sheet and shown alongside the latest reading |
| B4 | Vaccination record + **next-due reminders** | **P0** | ✅ | Tag was wrong — this was already complete: extends 0019's `vaccinations` table (batch number, visit link), `VaccinationPolicy` auto-computes next-due (+12mo), `PetDetailView`'s vaccination card color-codes overdue/due-soon (`Vaccination.dueStatus`) with a 1-tap "book vaccination visit" that opens `ServiceDetailView` pre-selected |
| B5 | Prescription history | P1 | ✅ | Same underlying feature as K2: `PetDetailView`'s prescription card lists a pet's `Prescription` rows (read-only `PrescriptionRepository`), vet-issued only via server-side RLS (no client insert policy at all). K2's remaining gap (a generated PDF of the prescription) is that row's concern, not this one's — "history" itself is fully shown |
...
| B8 | Deceased/rehomed pet handling | P1 | ✅ | Tag was wrong — this was already complete: soft-delete via `Pet.archivedAt`/`archiveReason`, excluded from `ManagePetsUseCase.list` (booking picker, vaccination nagging) by default, `PetDetailView`'s `.confirmationDialog` uses calm copy ("no longer with you", never "delete") and offers deceased/rehomed/other + unarchive |
...
| C7 | Map view of cluster coverage | P1 | ✅ | New `ServedCluster` model + `AddressRepository.listServedClusters` (Mock + Supabase, the latter via a `served_clusters` view over 0004's existing `circuit_cluster_centers` — the same table `match_cluster()` already geofences against, not a second source of truth) + `GetServedClustersUseCase` + `CoverageMapView` (MapKit circles/markers), reachable from `CircuitsListView`'s toolbar |
| C8 | Search (vet name, service, symptom) | P1 | ✅ | Tag was wrong — already complete: Postgres FTS (0025_search_fts.sql: generated `tsvector` + GIN on services/vets), `SearchUseCase`, `SupabaseCircuitRepository.searchCircuits`/`searchServices`, search bar on `CircuitsListView` matches vet name + service, not just area. "Symptom" search is served indirectly via matching service names/summaries, not a separate symptom taxonomy — no such taxonomy exists anywhere in this app |
| C9 | Recently viewed / rebook last visit (1 tap) | P1 | ✅ | Tag was wrong — already complete: `RecentlyViewedStore` (UserDefaults) backs a "Recently viewed" rail, `RebookLastVisitUseCase` (pulls the most recent *completed* visit, resolves its circuit) backs a 1-tap "Rebook last visit" card, both on `CircuitsListView` |
| C10 | Waitlist for uncovered clusters + "N neighbours waiting" | P1 | ✅ | Tag was wrong — already complete: `WaitlistEntry`/`WaitlistRepository`, `waitlist_count_near` RPC for the "N neighbours waiting" count, `AddressListView` shows a `WaitlistJoinRow` on every unserved address. C1's discovery empty-state now also routes here instead of dead-ending |
...
|---|---|---|---|
| D1 | Service catalog w/ categories (Consult, Vaccination, Grooming, Diagnostics, Deworming, Dental, Elder-care visit, Physio session) | **P0** | ✅ | Tag was wrong — already complete: `ServiceCategory` models all 8 categories (plus K6's `labTest`), `ServiceCatalogView` groups/filters by `ServiceCategory.allCases`, backed by `SupabaseCatalogRepository` reading real Postgres rows |
| D2 | Variants per service (duration/tier/package) | **P0** | ✅ | Tag was wrong — already complete: `ServiceVariant` (name/duration/price/additional-pet price/`isFollowUp`) on every `Service`, `ServiceDetailView` lets the customer pick one before adding to cart |
...
| D4 | Packages/bundles ("Puppy first-year: 4 visits + 3 vaccines") | P1 | ✅ | Real redemption tracking end-to-end: `Visit` now carries `serviceId`/`variantId`/`packageRedemptionId` (0062_package_redemptions.sql), and `BuyPackageUseCase` creates one `PackageRedemption` (total/used count) per `PackageItem`, tagging each expanded `CartItem` with it. Booking through `BookVisitUseCase`/`BookingCheckoutUseCase` atomically redeems the matching entitlement in the same `book_visit()` transaction as the slot-capacity check (mock: same guarantee via actor isolation) — a booking against an exhausted entitlement is rejected outright, not silently over-redeemed. `PackagesView`'s "My packages" section shows genuine "3 of 4 used" progress (`GetMyPackageRedemptionsUseCase`/`PackageRedemptionPolicy`), and checking out one line of a multi-visit package no longer discards the other, not-yet-booked lines (`CartView.settleCartAfterBooking`). Remaining, narrower gap: a checkout still only ever books one cart line at a time (the pre-existing "cart is one circuit/one slot" limitation `CartViewModel.primaryPetId` documents) — booking each of a package's remaining visits is N separate checkouts, not a single "redeem next" action, and the UI doesn't yet offer a dedicated "book against this entitlement" flow outside the cart |
| D5 | Per-vet service availability & per-vet pricing overrides | P1 | ✅ | `VetServiceOverride`/`VetServiceOverrideRepository` (Mock+Supabase) + `0032_vet_service_overrides.sql`, wired into `PricingEngine`/`GetQuoteUseCase` client-side; `create-quote` (the authoritative server-side quote) now reads the same `vet_service_overrides` table for the cart's circuit's vet — applies `price_override_minor_units` in place of the catalog price, and rejects the quote if the vet has opted out (`is_offered: false`) of a service in the cart |
...
| D7 | Catalog managed from ops console, not hardcoded | P0 | 🔨 | The app half is real and done: `SupabaseCatalogRepository`/`SupabasePackageRepository` read live from Postgres (`services`/`service_variants`/`addons`/`packages`), with an `is_admin()`-gated write policy (0003_catalog.sql) so an admin can edit the catalog directly (today via Supabase Studio's table editor, a genuine ops console) with zero app redeploy. What's missing is a bespoke ops-console UI for that editing — that lives in `admin-web`, which is out of this session's allowed scope (`VetCircuit/` + `backend/supabase/` only), so it stays a real, honest gap rather than something to fake here |
...
| E4 | Coupon / promo code entry + validation + stacking rules | P1 | ✅ | Tag was wrong — already complete: `validate_coupon()` RPC (0027_coupons.sql, server-side min-spend/usage-limit/per-user-limit/date-range checks — "stacking rules" here means one-coupon-per-cart validated against those limits, there's no concept of combinable coupons anywhere in the model to stack), `CartView`'s promo field wired through `ApplyCouponUseCase` into the signed quote, `PricingEngine`'s discount term |
| E5 | Wallet credits & loyalty point redemption at checkout | P1 | ✅ | Wallet-credit-at-checkout already worked (`useWalletBalance` toggle in `CartView`). Point redemption closed this pass: `LoyaltyRedemptionPolicy` (fixed conversion rate + minimum, unit-tested) + `LoyaltyRepository.redeemPoints`/`RedeemLoyaltyPointsUseCase` + a redeem row in `CartView` — redeemed points become wallet credit (spent via the existing toggle) rather than a second discount mechanism. Supabase side is a single `redeem_loyalty_points()` security-definer function (0050) doing both mutations atomically, the one narrow exception to `wallet_ledger`'s otherwise no-client-insert-at-all policy; a `SupabaseLoyaltyRepository` now exists too (loyalty was Mock-only before this pass) though it isn't yet wired into `DependencyContainer`, matching every other repository's current Mock-only wiring |
| E6 | **Server-authoritative quote**: `POST /quotes` returns a signed, TTL'd quote; order must reference a valid quote | **P0** | ✅ | Closed end-to-end this pass with one coordinating pipeline (`BookingCheckoutUseCase`, composing `BookVisitUseCase` + `StartCheckoutUseCase` + the new `VisitRepository.attachPayment`): `CartView`'s "Book & pay" gets a real signed `Quote` (refreshing an expired one), books the visit as pending payment against `cart.circuitId`/`slotId`, starts checkout, and on the checkout sheet's dismissal resolves the payment's actual status — confirming + linking the payment only on success, never on a client-asserted one. `BookingView` shares the same pipeline whenever the catalog flow handed it a specific service+variant (its "Get price & pay" path). Named remaining gaps: (1) `BookingView`'s generic "tap a circuit, no service chosen" entry point (`CircuitsListView`) has nothing to price a signed quote against, so it keeps the old direct-book-no-payment behavior; (2) a cart can hold multiple pets across its items, but `VisitRepository.createVisit` still books a single pet per visit, so checkout only books the first item's first pet (tracked on `CartViewModel.primaryPetId`) — both are scoped, named gaps, not silent ones |
| E7 | Slot **hold** (10 min) during checkout, auto-release | **P0** | ✅ | Tag was wrong — already complete: `BookingView` places a `SlotHold` (`HoldSlotUseCase`) the moment a slot is picked, shows a live countdown, releases it on confirm/dismiss, and re-holds if the slot selection changes; `expire_slot_holds()` (0006_slot_holds.sql) prunes stale holds server-side. The hold itself is UX (avoids someone else seeing it as free while this customer fills out the form) — the actual anti-oversell guarantee is `book_visit()`'s row-locked capacity check (0008_book_visit.sql), which holds regardless of whether a hold exists |
| E8 | Payment method choice: UPI intent, cards, netbanking, wallets, **pay-after-visit (cash/UPI to vet)** | P0 | ✅ | UPI/cards/netbanking/wallets are, by design, Razorpay hosted-checkout's own page (G1: "never raw card data" means the app never builds that picker itself) — now that a real checkout step exists end-to-end (E6), that part is genuinely reachable/covered by construction. Pay-after-visit is now a real, reachable second choice at the same decision point in `BookingView`/`CartView`: `Payment.Status.payAfterVisit` (0061_payment_pay_after_visit_status.sql widens the `payments` status CHECK), `BookingCheckoutUseCase.startPayAfterVisit`/`StartCheckoutUseCase.executePayAfterVisit` books and confirms the visit in one step against the same real, unexpired, signed `Quote` as the prepaid path (E6's quote-gating is never skipped) — no hosted-checkout URL, no `gatewayReference`, nothing to poll. `MarkPayAfterVisitCollectedUseCase` closes it out once the visit reaches `.completed`, mirroring how a gateway webhook marks a prepaid payment succeeded; `PaymentRetryPolicy`/G3 retry correctly never offers a retry on a `.payAfterVisit` payment. Remaining gap: `MarkPayAfterVisitCollectedUseCase` has no calling UI yet, since this app has no vet-facing surface at all (the same scope boundary as F9/I7) — it's exercised by tests today and by that future vet-side screen once it exists, not by a button a real vet can tap |
| E9 | Saved payment methods (gateway-tokenized, never stored by you) | P1 | ✅ | |
| E10 | Order confirmation screen + receipt email/SMS | P0 | 🔨 | The confirmation screen (`BookingConfirmedView`) and the receipt call (`SendTransactionalNotificationUseCase`, category `.visitConfirmed`) now fire off the *real* end of the booking pipeline in both `BookingView` and `CartView` — a receipt only ever goes out once a payment has actually succeeded and the visit is confirmed (E6), not merely requested. Stays 🔨 for the same reasons as before this pass: "SMS" is real (fallback decision + `sms_fallback_log`, per J8's own scope — no live gateway sends an actual text); "email" has no email-sending path anywhere in this codebase at all (no SMTP/provider integration exists to build on) |
| E11 | Tip the vet after visit | P2 | ✅ | Tag was wrong — already complete and reachable: `TipVetView` (preset + custom amounts) linked from `VisitDetailView`, `TipUseCase` (validated range, opens the tip checkout URL), credited 100% to the vet server-side via a trigger on `payments.kind = 'tip'` (0028_tips.sql) rather than the ~70% split a regular visit earns |
...
| G5 | GST-compliant invoice PDF per order | **P0** | ✅ | `InvoicePDFRenderer` typesets the server-issued `Invoice` row (invoice number, GST amount, line items — all server-computed, never client math) into a real PDF via `UIGraphicsPDFRenderer`; `InvoiceView` shows it in a `PDFView` with a share sheet, linked from `VisitDetailView` once a visit completes. No third-party PDF/gateway service needed — this is a system framework. |
| G6 | Wallet + double-entry ledger | P1 | ✅ | `wallet_ledger` (append-only, mirrors `vet_ledger`) + balance view + `CartView`'s "use wallet balance" toggle were already done; this pass closed the missing debit. `payment-webhook` — still the only writer of payment success, per the money discipline this whole area follows — now reads the succeeded payment's signed quote, and if its breakdown applied a "Wallet credit" line item, inserts the matching debit into `wallet_ledger` for that cart's user in the same request that confirms the visit. No client code ever inserts into `wallet_ledger` (it has no insert policy for `authenticated`/`anon` at all, unchanged) |
| G7 | **Vet payouts**: earnings view, weekly payout run, reconciliation | **P0 (partner)** | 🔨 | `0014_payouts.sql`'s `vet_ledger` (append-only, mirrors `wallet_ledger`) and `credit_vet_on_visit_completed()` already record each visit's ~70% split server-side. What's missing is everything this app itself could own: there is no vet-facing UI anywhere in this codebase — no vet-mode surface at all (same scope boundary as F9/I5/I7's blackout/OTP/checklist screens) — so an earnings view is structurally a separate vet-app workstream, not a gap in this customer app. The weekly payout run and its reconciliation are ops/infra (a scheduled job + gateway payout API), same category as G8. |
...
| H2 | Purchase via gateway **recurring mandate (UPI Autopay / e-mandate)** | P0 | 🔨 | `SubscribeToPlanUseCase`/`PaymentRepository.createCheckout(forSubscription:)` return a hosted-checkout URL, same as a one-time visit payment — there is no UPI Autopay/e-mandate-specific gateway call, because setting up a real recurring mandate requires an actual Razorpay (or equivalent) account and credentials this codebase has none of, same boundary as G1/G2's "mocked hosted-checkout URL flow" already noted elsewhere. Nothing more can be honestly built here without gateway access. |
...
| H4 | Renewal reminders (T-7, T-1) + receipt | P0 | ✅ | `RenewalReminderPolicy` (pure, tested) computes exactly which reminder is due; `RenewalReminderUseCase` sends it through the existing J8 notification pipeline, deduped per subscription+stage+day via `RenewalReminderDedupeRepository` (device-local, mirroring `NoShowDetectionRepository`/`PostVisitSummaryRepository`'s established shape), and `ManageSubscriptionView` shows a receipt (plan, amount, billed date) for the cycle. Closed the same way F4/I8 closed their equivalent gaps: `ProfileView`'s routine screen load (a tab every subscriber routinely passes through, not just subscription management) now checks and fires this on every appearance, not only when the subscription screen happens to be open. Honest, accepted limit, same as F4/I8: this is client-checked on app use, not a real daily server-side cron (plan §6.5) firing exactly at T-7/T-1 regardless of whether the app is opened that day. |
| H5 | Dunning: failed renewal → retry ladder → grace → downgrade | P1 | 🔨 | `DunningPolicy`/`DunningState` (pure, tested: +1/+3/+7 day retry ladder, then 7-day grace, then downgrade) are wired end to end via `DunningStatusUseCase`, and `ManageSubscriptionView` shows the customer a read-only "payment failed, retrying on/downgrades on <date>" banner. The grace→downgrade half is now closed the F4/I8 way: `ProfileView`'s routine load calls `resolveIfGraceExpired` on every appearance for any subscriber with dunning state, downgrading promptly once grace has quietly elapsed rather than only when they open subscription management — no separate dedupe needed since acting clears the dunning state itself. Still genuinely 🔨: the *other* half, `recordFailure`, is never called anywhere — a real renewal charge failure has no source in this codebase at all (no gateway is wired in), so there is no client-observable signal (unlike a no-show or a completed visit) to detect it from on any screen load. That entry point still needs a gateway webhook (plan §6.5), which is ops/infra work this app has nothing to hook into. |
...
| H7 | Corporate/RWA seat-based plan + seat assignment | P2 | ✅ | The seat-based plan itself (min. 5 seats, seat-count billing) already existed via `CorporatePlanView`/`SubscriptionManagementPolicy`. Seat *assignment* — who fills each billed seat — now ships too: `0048_corporate_seat_assignments.sql` (RLS scoped to the subscription owner) + `CorporateSeatAssignmentRepository`/`ManageCorporateSeatsUseCase` (tested: refuses once every seat is filled) + `CorporateSeatAssignmentView`, linked from `ManageSubscriptionView`. |

### I. The live visit (your trust moment)

| # | Capability | Pri | Status | Notes |
|---|---|---|---|---|
| I1 | Persistent "what's happening now" card on Home | P0 | ✅ | This app's IA has no separate Home tab (`MainTabView` is Book/Visits/Profile) — `VisitHistoryView` (the "Visits" tab) is where that role lives, and it already does it: a "Happening now" section pinned above history, with a pulsing live-indicator dot and "Vet is on the way" copy for the nearest non-terminal visit, tapping straight into `VisitDetailView`. |
| I2 | Status timeline w/ timestamps (requested → confirmed → assigned → en route → arrived → in progress → completed) | P0 | ✅ | `0046_visit_status_events.sql` adds an append-only, trigger-populated log of every status change (RLS select-only, no client write policy — mirrors `wallet_ledger`/`refunds`). `VisitTimelineView` renders it as a real per-step timeline with timestamps, linked from `VisitDetailView`'s status card, replacing the badge-only view this row used to describe. |
| I3 | **Live Activity + Dynamic Island** for "vet en route / ETA" | P1 | ✅ | `VetEnRouteAttributes` + `Activity<T>.request` call site wired from `LiveTrackingView`, and `VetCircuitWidget/VetEnRouteLiveActivity.swift` now renders it on the Lock Screen and Dynamic Island (expanded/compact/minimal). The widget extension target this previously needed already exists (added for N6's next-visit widget) — the gap this note used to describe is closed. |
...
| I8 | Post-visit summary push + in-app detail | P0 | 🔨 | In-app detail is done — `VisitDetailView`'s notes card plus the newly-added checklist (I7) and invoice (G5). `SendPostVisitSummaryUseCase` now sends the push too, through the existing J8 pipeline, the first time `VisitHistoryView` notices a completed visit. Still 🔨, honestly: the send is client-detected on next app open and de-duplicated with a device-local flag (`LocalPostVisitSummaryRepository`), not triggered server-side the instant a visit actually completes with a real server-side "already sent" record — that would need a backend job/webhook this codebase has no trigger point for. |
...
| J4 | **Masked voice calling** (Exotel/Twilio proxy — real numbers never exposed) | **P0** | 🔨 | Privacy + safety + "vet can't find the gate" reality. Full shape already exists end-to-end: `CallRepository.startCall` → `start-call` Edge Function → `call_sessions` (0013), returning only a proxy number, never a real one. Genuine remaining gap, structurally outside this pass: no Exotel/Twilio account or API key exists in this project (`EXOTEL_API_KEY`/`EXOTEL_VIRTUAL_NUMBER` unset), so the function falls back to a single shared placeholder number rather than actually provisioning a bridging call — see the function's own comment for exactly where the real gateway call goes once credentials exist |
...
| J6 | Video consult | P2 | 🔨 | No code exists for this at all (not even a stub, despite the prior note) — searched `VetCircuit/Domain`, `/Data`, `/Presentation` for any video-call model/repo/view and found none. Structurally out of scope for this pass: a real video consult needs a WebRTC/video SDK (Twilio Video, Agora, Daily, …) this project has no account or credentials for, the same class of gap as J4's telephony gateway — not something a mock repository can meaningfully stand in for the way masked calling's proxy-number shape can |
...
| K2 | Prescription (structured, vet-signed, PDF) | P1 | ✅ | `prescriptions` table + read-only `PrescriptionRepository`; structured data + vet-signed only via server-side RLS (no client insert policy). PDF gap closed: `Prescription.documentPDF` (`PDFDocumentBuilder`, native `UIGraphicsPDFRenderer` — no third-party library or account needed) renders a real single-page PDF client-side from the structured record, share-sheeted from a new share button on each `PetDetailView` prescription row |
...
| K4 | Vaccination certificate PDF + next-due auto-scheduling | P1 | ✅ | Next-due date auto-populates (+12mo, `VaccinationPolicy.suggestedNextDueDate`) when a vaccination is recorded — the "auto-scheduling" plan means. Certificate PDF gap closed: `Vaccination.certificatePDF` (same native `PDFDocumentBuilder` as K2, no third-party dependency) renders a real certificate client-side, share-sheeted from a new share button on each `PetDetailView` vaccination row |
| K5 | Follow-up booking in 1 tap (free follow-up window) | P1 | ✅ | `FollowUpBookingPolicy` (14-day window) gates a "Book free follow-up" button on `VisitDetailView`. Closed the "same vet/circuit not enforced" gap: it now resolves the visit's own `circuitId` via `CircuitRepository.circuit(id:)` and routes straight into `BookingView` (the direct circuit→slot booking flow, not the catalog/cart flow) with that circuit, the free follow-up variant, and the same pet all preselected — `BookingViewModel` gained `preselectedPetId` for this |
...
| L2 | Document-backed onboarding: degree, VCI cert, ID, police verification, photo | P0 | 🔨 | `VetOnboardingApplication` (5 required document URLs + submitted/underReview/approved/rejected) + `VetOnboardingRepository` (Mock/Supabase) + `SubmitVetOnboardingApplicationUseCase`, `vet_onboarding_applications` table (0046_vet_onboarding_applications.sql, applicant select/insert own row and update it only while `submitted`, admin select/update all, no delete for anyone). Marked 🔨 not ✅: this is fundamentally a vet-side workflow and this app is customer-facing only — there is no vet-facing UI surface here to build a real document-upload screen into, so the domain/data layer is complete and correct but unreachable from any screen in this app, a genuine scope boundary rather than a shortcut |
...
| L6 | Review moderation (profanity, PII, defamation) | P1 | ✅ | `ReviewModerationPolicy` (pure, unit-tested): word-list profanity check rejects submission, regex PII (email/phone/address) auto-redacted, defamation-risk phrasing sets `needsModeration` for human review without blocking. `SubmitReviewUseCase` runs it before ever calling the repository; `needsModeration`/`moderationFlags` threaded through `Review`, `ReviewRepository`, both implementations, and 0045_review_moderation.sql |
...
| N2 | Coupon campaigns (first-visit, win-back, cluster-launch) | P1 | ✅ | Seed campaigns FIRSTVISIT, WINBACK100 (0027_coupons.sql) and now CLUSTERLAUNCH (0047_cluster_launch_coupon.sql, additive), all validated via the same E4 `validate_coupon()` RPC; `MockCouponRepository` mirrors all three so the flow is exercisable without a backend |
| N3 | Lifecycle pushes: vaccination due, renewal, dormant 60d, abandoned cart | P1 | ✅ | Detection + queueing (`lifecycle-notifications` Edge Function, `notifications` table) plus drain: `DrainLifecycleNotificationQueueUseCase` fetches each user's unsent (`sent_at is null`) rows on `ProfileView`'s routine load, sends each through the J8 push/SMS-fallback pipeline, then marks it sent — same client-drain pattern as F4/I8/H4. Dedupe is server-side (`sent_at`, already permitted to update by the existing 0022 RLS policy), stronger than those three's device-local flags. Same accepted, honest limit as F4/I8/H4: no real server-side cron actually pushing on a schedule — it only drains when a subscriber happens to open their profile |
...
| N7 | Deep links + universal links for every campaign target | P1 | ✅ | `DeepLinkParser` (pure) + `.onOpenURL`; a shared `Router` (`App/Router.swift`, `@Observable`) now exists, holding `selectedTab` plus a typed-`Route` `NavigationPath` per converted tab (`Route.swift`: `.visitDetail`, `.chat(visitId:)`, `.household`). `vetcircuit://visit/<id>/chat` now pushes straight into that visit's chat thread (`VisitHistoryView`'s `.navigationDestination(for: Route.self)`) and `vetcircuit://household` pushes straight into `HouseholdView` (`ProfileView`'s), both switching tab *and* pushing the nested screen in one step — closing the exact "can't push a specific visit's chat from outside" gap this row previously called out. **Partial by design**: only the Visits and Profile tabs were converted to a Router-driven `NavigationStack(path:)`; the Book tab (`CircuitsListView`) still resolves `vetcircuit://book/<circuitId>` itself against its own loaded `[Circuit]` (unchanged since before this Router), since folding its `Circuit`-keyed `navigationDestination` plus search-result/rebook-sheet navigation into `Route` was judged a larger, riskier refactor than this pass should take on. Every other tab's `NavigationStack` (Book, and the ~15+ other screens with their own independent stacks) is untouched. Covered by `RouterHandleTests`/`DeepLinkParserTests` (pure Route-building logic; on-screen navigation isn't unit-testable in SwiftUI) |
...
| O4 | Accessibility: Dynamic Type to AX5, VoiceOver, Reduce Motion | P0 | 🔨 | Reduce Motion is genuinely wired into the shared animation primitives every screen builds on (`Theme.appearAnimation`, `Mascot`, `Confetti` all read `\.accessibilityReduceMotion` and skip/short-circuit their animation). This pass added a real, verified VoiceOver/Dynamic-Type audit on the app's 6 highest-traffic customer screens (mirroring O2's own core-flow priority list) — `CircuitsListView`, `ServiceDetailView`, `BookingView`, `CartView`, `VisitDetailView`, `ProfileView`: every previously-unlabeled icon-only toolbar/row control (coverage map, sort, filters, triage, cart, cart-item remove) now has `accessibilityLabel`/`accessibilityHint`; tappable card/rows that read as fragmented multi-stop VoiceOver elements (circuit rows, rebook/recently-viewed cards, variant/add-on/pet selectable rows, the vet-info card, the visit-status card, every `ActionRow` instance, the profile header, each pet row, the loyalty progress card) are now `accessibilityElement(children: .combine)` with `.isSelected` traits where relevant; the decorative loyalty progress bar is hidden from VoiceOver in favor of a spoken percentage. Confirmed all body/heading text on these 6 screens already uses Dynamic-Type-scalable styles (`.brandHeadline`/`.brandBody`/etc. wrap `Font.system(textStyle:)`, and plain `.headline`/`.title3` are likewise text styles) with no fixed-height frames wrapping text content found on these screens, so no AX5-clipping fixes were needed there. A second pass extended the same real coverage to 6 more high-traffic screens: `ChatView` (`arrow.up` send and `camera.fill` attach buttons labeled; `ChatBubble` now speaks as one combined element — "You: …" / "Photo message" / the other party's text — instead of separate bubble/read-receipt stops), `PetDetailView` (document-vault/lab-reports/medication-reminders `NavigationLink` cards combined with a spoken "Opens …" hint; vaccination/prescription row text groups combined while their share buttons stay independently tappable), `PaymentMethodsView` (the saved-card list row and the checkout picker's card row combined into one element each, speaking the card label plus its default/selected state), `PackagesView` (the "My packages" entitlement card and each `PackageCard`'s name/description and price/discount groups combined, with the redundant decorative `ProgressView` hidden in favor of its spoken progress label, buy button kept separate), the Support screens (`HelpCenterView`/`ContactSupportView` were already built from standard `Form`/`List`/`Label` rows with no icon-only or fragmented controls, so only a spoken-hours hint was added to "Call support"; `NotificationCenterView`'s notification rows combined into one element each with a spoken "Unread" prefix and mark-as-read hint, decorative unread dot hidden). Confirmed no fixed-height frames clip text content on any of these 6 screens either. This is now a 12-screen pass total, not a full audit: the remaining ~38 screens (vet detail, checkout, address management, timeline, live tracking, notification preferences, medication reminders, document vault, lab test reports, etc.) have not been checked for icon-only labels, row fragmentation, or AX5 clipping, and no VoiceOver rotor/focus-order pass has been done anywhere in the app — that is still the honest, named remaining gap, tracked here exactly as before rather than claimed as done |
