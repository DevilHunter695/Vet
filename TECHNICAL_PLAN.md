...
| H4 | Renewal reminders (T-7, T-1) + receipt | P0 | 🔨 | `RenewalReminderPolicy` (pure, tested) computes exactly which reminder is due; `RenewalReminderUseCase` sends it through the existing J8 notification pipeline, and `ManageSubscriptionView` shows a receipt (plan, amount, billed date) for the cycle. Still 🔨, honestly: it only runs when the subscription screen happens to be open — there's no daily scheduled job in this codebase (plan §6.5, ops/infra) to call it on T-7/T-1 regardless of whether the app is opened, which is what a real reminder needs. |
| H5 | Dunning: failed renewal → retry ladder → grace → downgrade | P1 | 🔨 | `DunningPolicy`/`DunningState` (pure, tested: +1/+3/+7 day retry ladder, then 7-day grace, then downgrade) are now wired end to end via `DunningStatusUseCase` — `recordFailure` advances the ladder, `resolveIfGraceExpired` performs the downgrade, and `ManageSubscriptionView` shows the customer a read-only "payment failed, retrying on/downgrades on <date>" banner. Still 🔨, honestly: nothing in this codebase *calls* `recordFailure` on a real renewal failure and nothing polls `resolveIfGraceExpired` on a schedule — both need a gateway webhook (renewal charge failed) and a cron job (plan §6.5) that no gateway integration exists here to drive; that trigger wiring is ops/infra work, not client-app logic. |
