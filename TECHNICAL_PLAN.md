...
| E8 | Payment method choice: UPI intent, cards, netbanking, wallets, **pay-after-visit (cash/UPI to vet)** | P0 | 🔨 | UPI/cards/netbanking/wallets are, by design, Razorpay hosted-checkout's own page (G1: "never raw card data" means the app never builds that picker itself) — now that a real checkout step exists end-to-end (E6), that part is genuinely reachable/covered by construction rather than theoretical. Stays 🔨 only for "pay-after-visit": it still isn't modeled anywhere (no field, no UI, no `Payment` case/status) — every booking that goes through the new pipeline is a prepaid hosted checkout. Adding a real pay-after-visit path needs a `Payment.Status`/visit-completion-triggered charge this codebase doesn't have yet, a materially different flow from "pay to book" |
